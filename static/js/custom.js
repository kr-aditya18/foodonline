    ...
    if ($('#id_search_location').length) {
        ...
        $('#id_search_location').autocomplete({
            ...
            select: function (event, ui) {
                ...
                $('#radius-warning').removeClass('show');
                return false;
            }
        });
        ...
        // ─── RADIUS-WITHOUT-LOCATION GUARD ─────────────────────────────────────
        // If the radius dropdown is invalid on its own without a set lat/lng,
        // the view has nothing to measure distance from and silently returns
        // every approved vendor. Block submission and show an inline warning
        // instead of letting that happen silently.
        var $homeForm = $('#home-search-form');
        if ($homeForm.length) {
            $homeForm.on('submit', function (e) {
                var radius = $('#radius').val();
                var lat    = $('#id_search_lat').val();
                var lng    = $('#id_search_lng').val();

                if (radius && (!lat || !lng)) {
                    e.preventDefault();
                    $('#radius-warning').addClass('show');
                    $('#id_search_location').focus();
                    return false;
                }
            ...
            // Clear the warning as soon as the user starts typing a location
            $('#id_search_location').on('input', function () {
                $('#radius-warning').removeClass('show');
            });

            // Clear the warning if radius is reset back to "Choose Radius"
